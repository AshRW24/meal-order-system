  ...
  await loginFormRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        ...
        userStore.setToken('session-based')
...
const handleRegister = async () => {
  ...
  await registerFormRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        await request.post('/admin/register', registerForm)
...
const handleForgetNext = async () => {
  if (forgetStep.value === 0) {
    ...
    try {
      const res = await request.get('/admin/security-question', {
...
const handleResetPassword = async () => {
  ...
  forgetLoading.value = true
  try {
    await request.post('/admin/reset-password', {
...
<style scoped>
/* Reuse app-wrapper for background */
.login-window {
  display: flex;
...
.login-card {
  width: 400px;
  padding: 30px;
  border-radius: 16px; /* Match window radius */
  box-shadow: 0 16px 48px rgba(0, 0, 0, 0.15); /* Stronger shadow */
}
